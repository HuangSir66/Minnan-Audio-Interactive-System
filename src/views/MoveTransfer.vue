<template>
  <div class="center-container-v" id='building-e'>
    <div class="alert-box-item-v">
        <div class="bigImg-div-v" @click="toGetVideo">
            <video class="bigImg-v" :src="valueUrl" v-if="valueUrl" controls></video>
        </div>
        <img
      ...
      if (event && event.target && event.target.files && event.target.files.length > 0) {
        ...
        if (!isLt20M) {
          ...
          reader.onload = () => {
            ...
            axios.post('http://localhost:8000/api/stylizer/vidio', formData)
              .then(response => {
                // 上传成功后的操作
                console.log('视频上传成功',response.data.stylized_video_url);
                this.valueUrl ='http://localhost:8000/'+ response.data.stylized_video_url
              })
              .catch(error => {
                // 上传失败后的操作
